...
const SOSSection = () => {
  ...
  const emergencyTypes = [
    { id: "cardiac", label: t("sosCardiac"), icon: Heart, color: "bg-destructive", description: t("sosCardiacDesc") },
    { id: "accident", label: t("sosAccident"), icon: Car, color: "bg-warning", description: t("sosAccidentDesc") },
    { id: "labor", label: t("sosLabor"), icon: Baby, color: "bg-sanjeevani-orange", description: t("sosLaborDesc") },
    { id: "other", label: t("sosOther"), icon: AlertTriangle, color: "bg-primary", description: t("sosOtherDesc") },
  ];
      ...
      <div className="container mx-auto px-4">
        <motion.div initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} className="text-center mb-16">
          <h2 className="text-4xl md:text-6xl font-bold text-foreground mb-6">
            {t("sosTitle1")} <span className="text-secondary">{t("sosTitle2")}</span>
          </h2>
          <p className="text-xl text-muted-foreground max-w-2xl mx-auto">{t("sosSubtitle")}</p>
        </motion.div>
          ...
          <>
            ...
            <motion.div initial={{ opacity: 0, scale: 0.9 }} whileInView={{ opacity: 1, scale: 1 }} viewport={{ once: true }} className="flex flex-col items-center">
              ...
              <p className="text-lg text-muted-foreground">
                {selectedType ? t("sosPressToAlert") : t("sosSelectType")}
              </p>
            </motion.div>
          </>
        ) : (
          <motion.div initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} className="max-w-2xl mx-auto">
            <Card className="p-10 shadow-elevated border-2 border-success">
              <div className="text-center mb-10">
                ...
                <h3 className="text-3xl font-bold text-foreground mb-3">{t("sosHelpOnWay")}</h3>
                <p className="text-lg text-muted-foreground">{t("sosDispatched")}</p>
              </div>
              <div className="bg-muted rounded-2xl p-8 mb-8">
                <div className="flex items-center justify-between mb-6">
                  <span className="text-lg font-medium text-muted-foreground">{t("sosEta")}</span>
              ...
              <div className="flex items-center gap-4 p-6 bg-card rounded-xl border border-border">
                ...
                <div className="flex-1">
                  ...
                  <p className="text-muted-foreground">{t("sosCertifiedWarrior")} • 4.9 ★</p>
                </div>
